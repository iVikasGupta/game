...
import { submitLevel2Decision, checkSubmissionStatus } from '../utils/api';
...
export const SubmitDecisionLevel2 = () => {
  ...
  const checkExistingSubmission = async () => {
    try {
      ...
      if (status.level2?.submitted) {
        setAlreadySubmitted(true);
        setPreviousResult(status.level2.result);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await submitLevel2Decision({
    ...
    return (
      <Layout>
        <div className="p-8 max-w-4xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900">Level 2 - Already Submitted</h1>
          </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6 mb-6">
            ...
            <p className="text-yellow-700">
              You have already submitted your decision for Level 2. Each player can only submit once per level.
            </p>
          </div>
          ...
          <button
            onClick={() => navigate('/submit-decision-level3')}
